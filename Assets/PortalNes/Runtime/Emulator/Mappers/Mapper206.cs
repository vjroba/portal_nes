...
{
    /// <summary>
    /// Namcot 108: MMC3-style bank registers with fixed PRG layout,
    /// fixed cartridge mirroring, and no IRQ or PRG RAM.
    /// </summary>
    public sealed class Mapper206 : IMapper
    {
        ...
        public Mapper206(byte[] prgRom, byte[] chrRom)
        {
            ...
            if (prgRom.Length < 4 * PrgBankSize || prgRom.Length % PrgBankSize != 0)
                throw new ArgumentException(
                    "Mapper 206 PRG ROM must contain at least four complete 8KB banks.",
                    nameof(prgRom));
            if (chrRom.Length < 8 * ChrBankSize || chrRom.Length % ChrBankSize != 0)
                throw new ArgumentException(
                    "Mapper 206 CHR ROM must contain complete 1KB banks.",
                    nameof(chrRom));
